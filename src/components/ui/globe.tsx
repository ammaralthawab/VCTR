...
interface GlobeProps {
  /** Size in px — defaults to 240 for standalone use */
  size?: number;
  className?: string;
}

const Globe: React.FC<GlobeProps> = ({ size = 240, className = "" }) => {
  // When a custom className is provided, let CSS fully control the dimensions
  const hasCssSizing = className.length > 0;
  ...
  return (
    <>
      ...
      <div
        ...
        style={{
          ...(hasCssSizing ? {} : { width: size, height: size }),
          flexShrink: 0,
          backgroundImage:
